...
#saveRDS(bacterial_shannons_div, file = "bacterial_shannons_div.RDS")

a <- which(metadata$Type == "Landrace")
early_data <- rel.data[a , ]
early_richness <- Richness[a]
early_metadata <- metadata[a , ]
early_shannons <- bacterial_shannons_div[a]
#saveRDS(early_shannons, file = "bacterial_early_shannons.RDS")
...
aggregate(bacterial_shannons_div, by = list(Category = metadata$group), FUN = mean)
aggregate(bacterial_shannons_div, by = list(Category = metadata$group), FUN = sd)
kruskal.test(bacterial_shannons_div ~ metadata$group)
dunnTest(bacterial_shannons_div ~ metadata$group, data = metadata)
...
#saveRDS(Early_Nmds, file = "Bacterial_Early_Nmds.RDS")

ggplot(data = Early_Nmds, aes( y = Early_Nmds2, x = Early_Nmds1, Type="p", colour = early_metadata$ALT)) + 
...
only_mexicana <- subset(data_metadata_early, Taxonomia== "Zea_mays_subsp._mexicana")
only_mexicana_data <- only_mexicana[,-(1:56)]
only_mexicana_data <- only_mexicana_data[,(1:(length(only_mexicana_data)-1))]

only_parviglumis <- subset(data_metadata_early, Taxonomia=="Zea_mays_subsp._parviglumis")
