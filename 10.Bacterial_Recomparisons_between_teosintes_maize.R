...
key_families <- c("Caulobacteraceae", "Comamonadaceae", "Enterobacteriaceae", "Rubritaleaceae", "Xanthomonadaceae")
...
ggplot(long_taxa_data, aes(x = variable, y = Abundance, 
  ...
  scale_fill_brewer(palette = "Set1") + ggtitle("A") +
  labs(y = "Rel. Ab. (%)", x = NULL, fill = NULL) + scale_fill_manual(values = c(x))

#saveRDS(long_taxa_data, file = "long_family_data_bacteria_maize_teo_comp.RDS")
...
kruskal.test(Wide.data.filtered$Caulobacteraceae, metadata$compar)
kruskal.test(Wide.data.filtered$Comamonadaceae, metadata$compar)
kruskal.test(Wide.data.filtered$Enterobacteriaceae, metadata$compar)
kruskal.test(Wide.data.filtered$Rubritaleaceae, metadata$compar)
kruskal.test(Wide.data.filtered$Xanthomonadaceae, metadata$compar)
...
Nmds <-cbind("Nmds1" = pcoaVS$vectors[,1], "Nmds2" = pcoaVS$vectors[,2]) #Makes a new sheet with dimension1 and 2
...
ggplot(rel.data, aes(x = metadata$compar, y = key_taxa, 
